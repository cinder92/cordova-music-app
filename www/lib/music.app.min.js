			...
			$localForage.getItem('songList').then(function(songs){
				if(undefined != songs && null != songs && songs.length > 0){
					...
					getAllSongsfromDevice().then(function(all){
						...
						/*console.log('device => '+all.length+', local => '+songs.length)
						if(all.length > songs.length){
							var newSongs = [];
							console.log('todas')
							//unir ambos arrays 
							var both = _.merge(all, songs);
							//y eliminar los duplicados
							var newSongs = _.uniqBy(both, 'Id');

							vm.list = newSongs
							//guardar en la base de datos la información de las canciones
		      				$localForage.setItem('songList',vm.list);
		      				//recorrer el array de las canciones "nuevas" y compararlo con las ya guardadas
		      				for(var i = 0; i < all.length; i++){
		      					
		      					/*if(undefined == songs[i] || all[i].Id != songs[i].Id || ){
		      						console.log(all[i])
		      						//todas las canciones que no se encuentren en la lista actual, se añaden
		      						newSongs.push(all[i])
		      					}
		      					if(undefined == songs[i]){
		      						console.log(all[i])
		      						newSongs.push(all[i])
		      					}
		      				}
		      				...
		      				//unir ambos arrays songs y newSongs
		      				var both = _.merge(newSongs,songs)
		      				
		      				$localForage.setItem('songList',both);

		      				$localForage.getItem('songList').then(function(list){
	      						console.log('locales',list.length,'both',both.length)
	      						vm.list = list
	      					})
						}*/
					})
